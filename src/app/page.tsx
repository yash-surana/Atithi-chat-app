...
export default function Home() {
    ...
    const handleVendorTypeChange = async (type) => {
        ...
        updateVendorType({ userId: loggedInUser._id, vendorType: type })
            .then((updatedUser) => {
                setLoggedInUser(updatedUser);
    ...
    if (loggedInUser) {
        if (!loggedInUser.isonboarding) {
			if((loggedInUser.role === "vendor" && loggedInUser.vendorType===null)|| loggedInUser.role === "user"){
            return (
                <main className="flex flex-col items-center justify-center min-h-screen p-4 text-black px-10 py-16" style={{ backgroundColor: '#f8ecde' }}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-lg w-full">
                        <div className="card flex flex-col items-center p-6 border-2 border-gray-900 shadow rounded-lg cursor-pointer " onClick={() => handleRoleChange("user")}>
                        ...
                        <div className="card flex flex-col items-center p-6 shadow rounded-lg cursor-pointer border-2 border-gray-900" onClick={() => setIsDialogOpen(true)}>
                    ...
                    <Dialog.Root open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                      ...
                      <button className="text-gray-500 hover:text-gray-700 transition-all duration-300 ease-out float-right">
                        <X className="w-6 h-6" />
                            ...
                            <Dialog.Title className="text-lg font-bold mb-4 ">Select Vendor Type</Dialog.Title>
